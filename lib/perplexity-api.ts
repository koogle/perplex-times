...
export class PerplexityAPI {
  private async makeRequest(
    messages: Message[],
    type: "article" | "headline" | "trending"
  ...
  async generateHeadline(
    topic: string
  ): Promise<{ headline: string; citations: string[] }> {
    const { content, citations } = await this.makeRequest(
      [
        {
          id: `headline-${new Date().toISOString()}`,
          role: "user",
  ...
  async generateArticle(headline: string): Promise<{
    ...
    const { content, citations } = await this.makeRequest(
      [
        {
          id: `article-${new Date().toISOString()}`,
          role: "user",
    ...
    const keywords =
      parts[1]?.split(",").map((k) => k.trim()) ||
      this.extractKeywords(articleContent);
  ...
  private extractKeywords(text: string): string[] {
    ...
    const uniqueWords = Array.from(
      new Set(words.filter((word) => word.length > 3 && !stopWords.has(word)))
    );
  ...
  async generateTrendingTopics(): Promise<{
    ...
    const { content, citations } = await this.makeRequest(
      [
        {
          id: `trending-${new Date().toISOString()}`,
          role: "user",
